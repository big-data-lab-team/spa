...
\section{Materials and Methods}\label{sec:methods}
    ...
    \tristan{add a paragraph to explain master, worker, driver}
    ...
    feature which provides fault-tolerance to the driver \tristan{, by means of checkpointing?}.
    This is particularly useful for
    pilot-based overlay clusters where the running driver may fail due to walltime expiration.
    Cluster deploy mode is not available for Python applications using the standalone mode \tristan{standalone mode wasn't defined.
    Rewrite positively saying that it's ony available in Scala?}.
    ...
    were any nodes can fail by design. \tristan{Last two sentences should be moved to job templates.}

    %%maybe add checkpointing

    \subsection{Job Templates}

    \tristan{This section could be renamed to ``SPA system design'', and you could add a paragraph explaining
    that for the sake of the experiment, you reimplemented the simplest possible pilot system since existing ones (refs)
    were either not found online, or difficult to use.}
    ...
    the first pilot to attempt to launch the driver, will do so \tristan{will do what? launch the driver?}.
    The reason for 
...
on B\'eluga and Cedar. In B\'eluga (Figure~\ref{fig:beluga1}) \tristan{why just in 1a and not 1b, ...?}, 16 pilots were always 
...
that we see a maximum speedup of 2.8, however, it generally remains that
pilots are slightly slower. \TG{You should report the average speed-up of
pilots: 8 pilots vs batch and 16 pilots vs batch.}
...
in makespans between batch and pilots \tristan{rather than other overheads?}, we compared makespan with queuing time of 
first pilot. As can be seen in Figure~\ref{fig:mqcedar}, variations in overall
makespans is primiarily due to queueing time. \tristan{I don't like these figures: if you want to show
this, you should plot the makespan vs the queuing time of the first pilot, in an XY plot. But then the MW plot
is better because it takes into account the queuing time of all pilots, not only the first one. I would remove this figure.}
...
\end{itemize}
\tristan{You should remove the ceil from the plot too.}
